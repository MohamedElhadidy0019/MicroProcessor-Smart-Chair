...
void setTarget_CM(double cm) { //convert cm to numsteps
  target = cm / (2 * PI * WheelRadius) * NUMSTEPS;
}
void setTarget_Angle(int angle) { //convert angle to numsteps
...
void Move(int kam, int mode = 0) { //move the wheen: mode = 0, cm   ; mode = 1 , angle
  if (mode == 0) setTarget_CM(kam);
  else if (mode == 1) setTarget_Angle(kam);
  MoveTargetSteps();
  //dir=!dir;
}
